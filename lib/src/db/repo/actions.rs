...
pub async fn upsert(pool: &Database, blog_id: Uuid, url: String) -> Result<Repo> {
    Ok(
        sqlx::query_file_as!(Repo, "src/db/repo/queries/upsert.sql", blog_id, url)
            .fetch_one(pool)
            .await?,
    )
}
...
pub async fn set_metadata(pool: &Database, repo_id: Uuid, metadata: Value) -> Result<Repo> {
    Ok(sqlx::query_file_as!(
        Repo,
        "src/db/repo/queries/set_metadata.sql",
        repo_id,
        metadata,
    )
    .fetch_one(pool)
...
pub async fn get_by_url(pool: &Database, url: &str) -> Result<Repo> {
    Ok(
        sqlx::query_file_as!(Repo, "src/db/repo/queries/get_by_url.sql", url)
            .fetch_one(pool)
            .await?,
    )
}
