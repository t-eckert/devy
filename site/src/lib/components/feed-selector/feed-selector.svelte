...
<div class="flex flex-col col-start-1 sm:flex-col items-start gap-2">
	{#each feedConfigs as feedConfig}
		<a href={`/feeds/${feedConfig.id}`} class={style(feedConfig.id, selectedFeed)}
			>{feedConfig.title}</a
		>
	{/each}
</div>
