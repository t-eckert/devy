...
const Byline: React.FC<Props> = ({ postMetadata }) => {
  ...
  return <div className="w-full max-w-sm p-2 border rounded-md flex flex-col items-start gap-0.5 text-sm text-neutral-800">
    <div className="font-medium">{author.name ?? author.username}</div>
    {createdAt === updatedAt ?
      <div>Published {(new Date(createdAt)).toLocaleDateString()}</div> :
      <div>Updated {(new Date(updatedAt)).toLocaleDateString()}</div>
    }
    <div className="mt-0.5 flex flex-row flex-wrap gap-1">
